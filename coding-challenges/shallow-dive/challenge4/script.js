...
//Below function citiesInState() finds the list of cities for a particular state

function citiesInState(inputState) {
    var cityListInState = [];
    for (var i=0; i<cities.length; i++){
        if (inputState === cities[i]['state']) {
            cityListInState.push(cities[i]['name'])
        } else continue;
    }

    console.log(inputState + " has these cities: ");
    for(var i=0; i<cityListInState.length; i++){
        console.log(cityListInState[i]);
    }

}

citiesInState("Maharashtra");
citiesInState("Jharkhand");
citiesInState("Himachal Pradesh");


//Below function cityFinder() finds the state in which an input city is.

function cityFinder(inputCity){
    for (var i=0; i<cities.length; i++){
        if (inputCity === cities[i]['name']) {
            var outputState = cities[i]['state']
        } else continue;
    }
    console.log(inputCity + " is in the state of " + outputState);
    // return outputState;
}

//
// cityFinder("Vasai-Virar");
// cityFinder("Surat");
// cityFinder("Raipur");
// cityFinder("Ranchi");
// cityFinder("Tiruchirappalli");
